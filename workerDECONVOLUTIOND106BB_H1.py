#conda activate cellpose2&&python D:\Carlos\Scripts\workerDECONVOLUTIOND106BB_H1.py
#
######### Please add documentation
### This is experiemnt D106, we are adding all the hybs together to have some data to present. Started 10/23/23 @ 11:33 AM
...
#S:\20230908_D106Luo\DNA\AnalysisDeconvolveCG
save_folder = r'S:\20230908_D106Luo\DNA\AnalysisDeconvolveCG'
...
def main_f(set_ifov,try_mode=True):
    ...
    #S:\20230908_D106Luo\DNA (this is on merfish XI)
    all_flds_ = glob.glob(r'R:\20230908_D106Luo\DNA\H*B*'+set__)
    all_flds_ += glob.glob(r'S:\20230908_D106Luo\DNA\H*B*'+set__) ################################# modify for 3-col small tad 
    all_flds_ = np.random.permutation(all_flds_) ##causing issues on the whole
    redefine_color += [[0,1,2,3]]*len(all_flds_) #### three signal color
    
    all_flds+=list(all_flds_) #potential fix is to cast it as a list
    ...
    '''
    #note, 103,104,105,106 is being excluded rn due to it being transferred
    #all_flds = [s for s in all_flds if "H103" not in s]
    #all_flds = [s for s in all_flds if "H104" not in s]
    #all_flds = [s for s in all_flds if "H105" not in s]
    #all_flds = [s for s in all_flds if "H106" not in s]
    
    #print([os.path.basename(fld) for fld in all_flds])
    ...
    if not os.path.exists(fovs_fl):
        ...
        fovs = list(np.random.permutation(fovs)) ## this is scrambling the FOVS
    if ifov<len(fovs):
        fov = fovs[ifov]
        print(fovs)
